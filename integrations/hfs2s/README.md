...
APIMart uses `gpt-image-2` with one PNG reference and `n: 1`. Result hosts are limited to `upload.apimart.ai` and `getapib.org`, the latter observed in authenticated provider results. Unknown image hosts fail visibly rather than allowing arbitrary fetches. Retrieving a completed image does not submit another generation.
